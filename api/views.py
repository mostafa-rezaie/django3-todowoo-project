...
class TodoUpdateDestroyRetrieve(generics.RetrieveUpdateDestroyAPIView):
    serializer_class = TodoSerializer
    permission_classes = [permissions.IsAuthenticated]
...
def signup(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'error', 'username has been taken'}, status=201)


class getTheToken(generics.ListAPIView):
    permission_classes = [permissions.IsAuthenticated]
